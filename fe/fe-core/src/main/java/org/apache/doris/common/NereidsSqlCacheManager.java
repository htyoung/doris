...
public class NereidsSqlCacheManager {
    ...
    private String generateCacheKey(ConnectContext connectContext, String sqlOrMd5) {
        String currentCatalog = Optional.ofNullable(connectContext.getCurrentCatalog()).map(CatalogIf::getName)
                .orElse("");
        String currentDb = Optional.ofNullable(connectContext.getDatabase()).orElse("");
        return currentCatalog + "." + currentDb + ":" + connectContext.getCurrentUserIdentity().toString() + ":"
                + sqlOrMd5;
    }
    ...
    private TableIf findTableIf(Env env, FullTableName fullTableName) {
        ...
        return db.flatMap(tableIfDatabaseIf -> tableIfDatabaseIf.getTable(fullTableName.table))
                .orElse(null);
    }
